...
class Deserteur(commands.Cog):
    ...
    async def deserteur(
        ...
        personne: nextcord.Member = SlashOption(
            description="Personne qui a déserté.", required=True
        ),
    ):
    ...
    @slash_command(
        name="desertions",
        description="Affiche le nombre de désertions total de la personne choisie",
    )
    async def desertions(
        self,
        interaction: Interaction,
        personne: nextcord.Member = SlashOption(
            description="Personne dont on veut voir le nombre de désertions.", required=True
        ),
    ):
        await interaction.response.defer()
        if not await verif_guild(interaction):
        ...
        await interaction.followup.send(
            embed=embed_warning("", f"{personne.mention} a déjà déserté `{desertions}` fois !")
        )
...
async def pardon(interaction, personne):
    ...
    setattr(
        result_deserteur, "dernier_pardon", datetime.datetime.now(tz=pytz.timezone("Europe/Paris"))
    )
