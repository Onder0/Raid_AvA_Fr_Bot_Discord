...
class Stream(commands.Cog):
  ...
  @slash_command(name="stream", description="Permet à une personne de streamer dans les canaux de raids !")
  async def stream(self, interaction : Interaction,
    personne : nextcord.Member = SlashOption(description="La personne qui pourra streamer.", required=True) 
  ):
    await interaction.response.defer()
    logger.info(f"{interaction.user.id}: {interaction.user.display_name} exécute la commande /stream {personne.display_name} .")
    
    if not await verif_guild(interaction) : return
    chan_commande =  interaction.guild.get_channel(settings.chan_commandes)
    if not await verif_chan(interaction, chan_commande) : return

    streamer = nextcord.utils.get(interaction.guild.roles, id = settings.streamer)

    await personne.add_roles(streamer)
    await interaction.followup.send(embed=embed_success("",f"{personne.mention} est {streamer.mention} jusqu'à 6h !"))
