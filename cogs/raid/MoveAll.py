...
class MoveAll(commands.Cog):
    ...
    @slash_command(
        name="moveall",
        description="Déplace les personnes du channel où vous vous trouver vers celui choisi.",
    )
    async def moveall(
        self,
        interaction: Interaction,
        channel: nextcord.VoiceChannel = SlashOption(
            description="Channel dans lequel vous voulez aller.", required=True
        ),
    ):
        ...
        if interaction.user.voice is None:
            error_msg = await interaction.followup.send(
                embed=embed_error(
                    "",
                    "Vous devez être connecté à un canal vocal pour utilisé la commande /moveall",
                )
            )
            logger.warning(
                f"Échec: {interaction.user.display_name} n'était pas connecté à un vocal.\n"
            )
        ...
        if unBonus == "":
            await interaction.followup.send(
                embed=embed_success("", f"Le move all a été effectué avec succès !")
            )
        else:
            await interaction.followup.send(
                embed=embed_warning(
                    "",
                    f"Le bonus a été retiré à {unBonus}.\n**Le move all a été effectué avec succès !**",
                )
            )
