...
class Bonus(commands.Cog):
  ...
  async def bonus(self, interaction : Interaction, 
    personne: nextcord.Member = SlashOption(description="Personne à laquelle donner le 🌟 Bonus 🌟 !", required=False)
  ):
    await interaction.response.defer()
    if personne == None :
        logger.info(f"{interaction.user.id}: {interaction.user.display_name} execute la commande /bonus.")
    else :
    ...
    chan_attente = nextcord.utils.get(interaction.guild.voice_channels, id = settings.chan_attente_bonus)
    ...
    if personne:
      ...
      await interaction.followup.send(embed=embed_success("",f"Les {role.mention} ont bien été attribués."))
